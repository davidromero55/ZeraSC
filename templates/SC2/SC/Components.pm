...
sub component_carousel{
  my $self = shift;
  return $self->render_template($vars);
}
sub component_categorypromo{
  my $self = shift;
  my $categories = $self->selectall("SELECT category_id, lower(name) as name, image, url_key FROM sc_categories WHERE active = 1  and parent_id = 0 ORDER BY sort_order limit 3");
  my $vars = {
      categories  => $categories,
  };
  return $self->render_template($vars);
}
sub component_newproducts{
  my $self = shift;
  my $products = $self->selectall("SELECT name, image, code, sale_unit FROM sc_products ORDER BY  added_on limit 4");
  my $vars = {
    products  => $products,
  };
  return $self->render_template($vars);
}
sub component_category{
  my $self = shift;
  my $categories = $self->selectall("SELECT category_id, lower(name) as name, image, url_key FROM sc_categories WHERE active = 1  and parent_id = 0 ORDER BY sort_order");
  my $vars = {
      categories  => $categories,
  };
  return $self->render_template($vars);
